...
Exercise questions are marked with the symbol =Q?=. If they contain the keyword *advanced*, then it is safe to skip them.
...
To install it, head over [[https://doc.sagemath.org/html/en/installation/index.html][here]] (though it might not work /during/ the exercise session).
** 2.3) sboxU
[[https://github.com/lpp-crypto/sboxU][sboxU]] is a library specifically intended to study S-boxes, that you can =import= from SAGE once you have properly installed it. Instructions to this end are available on its repository ([[https://github.com/lpp-crypto/sboxU][here]]). Unlike "regular" Python programs, =sboxU= must be compiled: large chunks of it are written in =C++= for speed. We will see how that works on Friday (see [[https://github.com/lpp-crypto/FSE2025-spring-school/tree/main/Tools#adding-some-c-to-it][here]]).

** TODO 2.4) Notebooks and scripts
...
- Q? :: Compute the lookup table of the 4-bit permutation $(x_0, ..., x_3) \mapso (x_0, x_0+x_1, x_3, x_3)$ using bit fiddling (i.e., masks and shifts), and using a matrix multiplication.
...
- Q? :: What is the differential uniformity of the monomial $x \mapsto x^{14}$ in the field $F_{2^n}$ for $n \in \{4, ..., 10\}$? What about the polynomial $x \mapsto x^{28}^{} + x^4 + x$? For the second question, you can both implement a small script, and use paper and pencil using your results to the first question.
...
- Q? :: rebuild the AES S-box by hand, see for example [[https://en.wikipedia.org/wiki/Rijndael_S-box][wikipedia]] for its definition.
...
** Recover an equivalent permutation
- Q? :: What are the main cryptographic properties of this function? Does this Walsh spectrum ring a bell? What about its differential uniformity?
- Q? :: Generate the Jackson Pollock representation of its LAT. Check the distribution of the coefficients in each column of the LAT. Which column indices do not contain an 8 (or -8)? What structure does this set have?
- Q? (advanced) :: Build a linear permutation such that applying to the column indices will put all the "strange" ones to the left. How can you use this linear permutation andthe =dillon= lookup table to build another S-box that has this even more structured LAT?

** Decomposing the new permutation
...
- Q? :: What pattern can you spot in the restriction of this function to all the sets of the form $S_c = \{(x || c), x \in F_2^3\}$, where $c$ takes all possible values? Hint: split the output into two halves.
...
- Q? :: How are the $T_c$ related to one another? Try plotting their DDT/LAT to get an idea.
-  :: 

* TODO 6) More advanced functionalities of sboxU
** TODO 7.1) FastLinearMapping
** TODO 7.2) Easy composition and LUT building
** TODO 7.3) Affine equivalence
