...
We can then use this software stack to experimentally test the evolution of the algebraic degree of an SPN with a specific S-box. Consider S-box or =PRESENT=, along with the same linear linear as before, namely =L= which maps =(x0, x1, x2, x3)= to =(x0^t, x1^t, x2^t, x3^t)=, where =^= is the XOR, and where =t = x0^x1^x2^x3=. Finally, consider the operation =K(x, i)= that XOR =2**i= into a 16-bit vector =x=.

- Q? :: What is the algebraic degree of each coordinate of =r_0 = K_0 o L o S=? of =r_1 o r_0=? How many rounds to you need to reach  a full degree on all the coordinates? You can go further and compare this with your results from the previous exercise sessions.
