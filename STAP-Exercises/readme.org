...
** CONTINUE Declaring a system of multivariate equations
The =polynomial_ring= method of the =GF= class can conveniently take a list as input. We can thus write the following. Note the use of =.gens()= instead of =.gen()= !

#+BEGIN_SRC python
X, Y = GF(2**31-1).polynomial_ring(["X", "Y"]).gens()
print(X, Y)
print(X*Y**2 + X + 3*Y + 2)
#+END_SRC
...
- round function :: =Cobaye= uses a Generalized Feistel Network structure, combined with a diffusion layer.
  ...
  2. The linear layer L operates as follows.
     ...
     + y_{2i} += 2u
     + y_{2i+1} -= 2u
- reference implementation :: A reference implementation in C++[fn:1] that is unlikely to compile properly is provided below. 
